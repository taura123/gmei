            ...
            <div className="container mx-auto px-4 md:px-6">
                <div className="text-center max-w-4xl mx-auto mb-12 md:mb-16 space-y-4">
                    ...
                    <p className="text-base md:text-lg text-muted-foreground font-medium max-w-2xl mx-auto leading-relaxed px-4">
                        Kami melayani berbagai segmen dalam ekosistem pendidikan dan dunia kerja, dengan produk serta layanan yang relevan.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 md:gap-6 max-w-7xl mx-auto">
                    {segments.map((segment, index) => (
                        <div
                            key={index}
                            className={`group flex flex-col items-center text-center py-8 px-6 rounded-[2rem] md:rounded-[2.5rem] border border-slate-200 dark:border-slate-800 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 bg-card text-card-foreground min-h-[320px] md:min-h-[340px]`}
                        >
                            <div className="mb-6 relative h-16 w-20 overflow-hidden transition-transform duration-500 group-hover:scale-110">
                                <Image src={segment.icon} alt={segment.title} fill className="object-contain" />
                            </div>
                            <h3 className="text-lg font-black text-slate-900 mb-3 leading-tight uppercase tracking-tight">{segment.title}</h3>
                            <p className="text-[13px] text-muted-foreground font-medium mb-6 leading-relaxed">
                                {segment.description}
                            </p>
                            <Link href={segment.link} className="mt-auto">
                                <span className="text-orange-500 font-black text-[11px] uppercase tracking-widest underline underline-offset-[10px] decoration-1 hover:text-orange-600 transition-colors">
                                    Lihat Produk
                                </span>
                            </Link>
                        </div>
